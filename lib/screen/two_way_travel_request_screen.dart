...
class TwoWayTravelRequestScreen extends StatefulWidget {
  const TwoWayTravelRequestScreen({super.key});

  @override
  State<TwoWayTravelRequestScreen> createState() => _TwoWayTravelRequestScreenState();
}

class _TwoWayTravelRequestScreenState extends State<TwoWayTravelRequestScreen> 
  ...
  // Controllers for onward journey
  final onwardFromCtrl = TextEditingController();
  final onwardToCtrl = TextEditingController();
  final onwardDateCtrl = TextEditingController();

  // Controllers for return journey
  final returnFromCtrl = TextEditingController();
  final returnToCtrl = TextEditingController();
  ...
  String purpose = "";
  String onwardMode = "Flight";
  String returnMode = "Flight";
  ...
  Future<void> submit() async {
    ...
    // Validate return date is after onward date
    if (returnDateCtrl.text.isNotEmpty && onwardDateCtrl.text.isNotEmpty) {
      final onwardDate = DateTime.parse(onwardDateCtrl.text);
      final returnDate = DateTime.parse(returnDateCtrl.text);
      
      if (returnDate.isBefore(onwardDate)) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Return date must be after departure date'),
    ...
    try {
      final message = await TravelRequestService.submitTwoWayTravelRequest(
        travelType: travelType,
        travelFunding: travelFunding,
        purpose: purpose,
        onwardFrom: onwardFromCtrl.text.trim(),
        onwardTo: onwardToCtrl.text.trim(),
        onwardMode: onwardMode,
        onwardDate: onwardDateCtrl.text.trim(),
        returnFrom: returnFromCtrl.text.trim(),
        returnTo: returnToCtrl.text.trim(),
        returnMode: returnMode,
        returnDate: returnDateCtrl.text.trim(),
          ...
          children: options.map((option) {
            ...
            return ChoiceChip(
              label: Row(
                ...
                children: [
                  ...
                  Text(
                    option['label'],
                    style: TextStyle(
                      fontSize: isSmall ? width * 0.03 : width * 0.035,
                      fontWeight: FontWeight.w600,
              ...
              padding: EdgeInsets.symmetric(
                horizontal: isSmall ? width * 0.03 : width * 0.04,
                vertical: isSmall ? height * 0.008 : height * 0.012,
              ),
            );
          }).toList(),
        ),
  ...
  Widget _buildLocationRow({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isDarkMode ? Colors.grey[700]! : skyBlue.withOpacity(0.3)!,
          width: 1,
        ),
      ),
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(
                title.contains("Onward") ? Icons.flight_takeoff : Icons.flight_land,
                size: width * 0.05,
                color: skyBlue,
              ),
              SizedBox(width: width * 0.02),
  ...
  void dispose() {
    ...
    onwardFromCtrl.dispose();
    onwardToCtrl.dispose();
    onwardDateCtrl.dispose();
    returnFromCtrl.dispose();
    returnToCtrl.dispose();
    returnDateCtrl.dispose();
    descCtrl.dispose();
    super.dispose();
  }
                      ...
                      child: Container(
                        ...
                        child: Transform.translate(
                          ...
                          child: Opacity(
                            ...
                            child: Column(
                              children: [
                                // Header Section with Sky Blue Gradient
                                Container(
                                  padding: EdgeInsets.symmetric(
                                    horizontal: width * 0.04,
                                    ...
                                    children: [
                                      Row(
                                        ...
                                        children: [
                                          ...
                                          SizedBox(width: width * 0.06), // For balance
                                        ],
                                      ),
                                      SizedBox(height: height * 0.01),
                                      Container(
